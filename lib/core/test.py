...
def im_detect_all(model, im, box_proposals=None, timers=None, proposal_eval=False):
    ...
    if proposal_eval:
        ...
        boxes = np.zeros((rois.shape[0], 8))
        scores = np.zeros((rois.shape[0], 2))
        boxes[:, 4:8] = rois[:, 1:5]
        scores[:, 1] = rois[:, 0]
    else:
        if cfg.TEST.BBOX_AUG.ENABLED:
            ...
            scores, boxes, im_scale, blob_conv = im_detect_bbox(
                model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    # scores.shape: 1000*81, boxes.shape:1000*324
    ...
    scores, boxes, cls_boxes = box_results_with_nms_and_limit(scores, boxes, proposal=cfg.TEST.TEST_RPN,
                                                              start_cls=start_cls, nms_thresh=nms_thresh)
    ...
    return cls_boxes, cls_segms, cls_keyps, blob_conv, im_scale
...
def im_detect_bbox_crop(model, inputs, im_scale, crop_size, chip_start):
    """Prepare the bbox for testing"""
    startx, starty = chip_start
    im_shape = ((starty + crop_size) / im_scale[0], (startx + crop_size) / im_scale[0])
    ...
    if cfg.MODEL.FASTER_RCNN:
        rois = return_dict['rois'].data.cpu().numpy()
        batch_size = len(im_scale)
        for i in range(batch_size):
            idx = np.where(rois[:, 0] == i)[0]
            rois[idx, 1] = rois[idx, 1] + startx
            rois[idx, 2] = rois[idx, 2] + starty
            rois[idx, 3] = rois[idx, 3] + startx
            rois[idx, 4] = rois[idx, 4] + starty
    ...
    if cfg.TEST.BBOX_REG:
        ...
        if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            ...
            box_deltas = box_deltas.view(-1, 4) * cfg.TRAIN.BBOX_NORMALIZE_STDS \
                         + cfg.TRAIN.BBOX_NORMALIZE_MEANS
        pred_boxes = box_utils.bbox_transform(boxes, box_deltas, cfg.MODEL.BBOX_REG_WEIGHTS) # x1y1x2y2
...
def im_detect_bbox_aug(model, im, box_proposals=None):
    ...
    if cfg.TEST.BBOX_AUG.CROP:
        ...
        # crop_size = cfg.TEST.BBOX_AUG.CROP_SIZE
        crop_size=1024
        original_inputs, im_scale = _get_blobs(im, rois=None, target_scale=scale, target_max_size=max_size)
        height, width = original_inputs['data'].shape[-2::]
        numx = width // crop_size + 1
        deltax = (numx * crop_size - width)/(numx - 1)
        numy = height // crop_size + 1
        deltay = (numy * crop_size - height)/(numy - 1)
        for i in range(numy):
            for j in range(numx):
                chip_startx, chip_starty = map(int, [(crop_size - deltax)*j, (crop_size - deltay)*i])
                chip_inputs = {'data':original_inputs['data'][:, :,
                                      chip_starty:chip_starty+crop_size,
                                      chip_startx:chip_startx+crop_size,],
                               'im_info': [[crop_size, crop_size, info[2]] for info in original_inputs['im_info']]}
                chip_inputs['im_info'] = np.array(chip_inputs['im_info'])

                chip_start = (chip_startx, chip_starty)
                scores_chip, boxes_chip = im_detect_bbox_crop(
                    model, chip_inputs, im_scale, crop_size, chip_start
                )
                add_preds_t(scores_chip, boxes_chip)
        im_scale_i = im_scale
        blob_conv_i = None
    else:
...
def box_results_with_nms_and_limit(scores, boxes, proposal=False, start_cls=1, nms_thresh=cfg.TEST.NMS):  # NOTE: support single-batch
    ...
    for j in range(start_cls, num_classes):
        ...
        if cfg.TEST.SOFT_NMS.ENABLED:
            ...
            keep = box_utils.nms(dets_j, nms_thresh)
...
def im_detect_rois(model, im, target_scale, target_max_size, boxes=None):
    """Prepare the bbox for testing"""
    ...
    # unscale back to raw image space
    rois[:, 1:5] = rois[:, 1:5] / im_scale

    # TODO:sort rois according to1 score
    sorted_idx = np.argsort(rois[:, 0])
    sorted_idx = sorted_idx[::-1]
    rois = rois[sorted_idx, :]
