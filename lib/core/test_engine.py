...
def test_net(
    ...
    # # for DEBUG
    # roidb, _, _ = combined_roidb_for_training(
    #     dataset_name, [])
    ...
    for i, entry in tqdm(enumerate(roidb)):
        ...
        # flag = 0
        # while (flag != 1):
        #     i = np.random.randint(0, len(roidb))
        #     entry = roidb[i]
        #     for box in entry['boxes']:
        #         area = (box[3] - box[1]) * (box[2] - box[0])
        #         if area <= 32*32:
        ...
        cls_boxes_i, cls_segms_i, cls_keyps_i, blob_conv, im_scale = im_detect_all(model, im, box_proposals, timers, proposal_eval=proposal_eval)
        ...
        if cfg.VIS:
            print('visualizing {}/{}...'.format(i, end_ind))
...
def initialize_model_from_cfg(args, gpu_id=0):
    ...
    if args.load_ckpt:
        ...
        net_utils.load_ckpt(model, checkpoint['model'], strict=True)
...
def test_RPN(
    ...
    for i, entry in enumerate(roidb):
        ...
        im = cv2.imread(entry['image'])
        cls_boxes_i, cls_segms_i, cls_keyps_i = im_detect_rois(model, im, box_proposals, timers)
        extend_results(i, all_boxes, cls_boxes_i)
