...
class fpn(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.P2only:
            ...
            # use all levels
            C1 = self.conv_body.res1(x, C1=True)
            C1_2 = self.conv_body.res2(C1)
            fpn_inner2 = fpn_inner_blobs[-1]
            #C2 = conv_body_blobs[1]
            fpn_inner3 = fpn_inner_blobs[-2]
            # return fpn_output_blobs, C1, fpn_inner2
            return fpn_output_blobs, C1_2, fpn_inner2, fpn_inner3
...
class fpn_rpn_outputs(nn.Module):
    ...
    def __init__(self, dim_in, spatial_scales):
        ...
        if cfg.FPN.EXTRA_ANCHOR_LAYER:
            sc = self.spatial_scales[k_max - k_min]  # in reversed order
            lvl_anchors = generate_anchors(
                stride=2. ** k_min,
                sizes=(cfg.FPN.RPN_ANCHOR_START_SIZE * 2. ** (-1),),
                aspect_ratios=cfg.FPN.RPN_ASPECT_RATIOS
            )
            self.GenerateProposals_modules.append(GenerateProposalsOp(lvl_anchors, sc))
            self.EXTRA_LAYER_CONV = nn.Conv2d(self.dim_in, self.dim_in, 3, 1, 1)
    ...
    def _init_weights(self):
        if cfg.FPN.EXTRA_ANCHOR_LAYER:
            init.normal_(self.EXTRA_LAYER_CONV.weight, std=0.01)
            init.constant_(self.EXTRA_LAYER_CONV.bias, 0)
    ...
    def forward(self, blobs_in, im_info, roidb=None):
        ...
        if not cfg.FPN.EXTRA_ANCHOR_LAYER:
            assert len(blobs_in) == k_max - k_min + 1
            start_k = k_min
        else:
            start_k = k_min - 1
            bl_extra = F.relu(self.EXTRA_LAYER_CONV(blobs_in[k_max - k_min]), inplace=True)

        for lvl in range(start_k, k_max + 1):
            slvl = str(lvl)
            if lvl == k_min - 1:
                bl_in = bl_extra
            else:
            ...
            if not self.training or cfg.MODEL.FASTER_RCNN:
                ...
                fpn_rpn_rois, fpn_rpn_roi_probs = self.GenerateProposals_modules[lvl - start_k](
                    fpn_rpn_cls_probs, fpn_rpn_bbox_pred, im_info)
                # print(fpn_rpn_rois.shape) # 1252*5
                # print(fpn_rpn_roi_probs.shape) #1252 *1
...
def fpn_rpn_losses(**kwargs):
    ...
    losses_bbox = []
    if cfg.FPN.EXTRA_ANCHOR_LAYER:
        start_k = cfg.FPN.RPN_MIN_LEVEL - 1
    else:
        start_k = cfg.FPN.RPN_MIN_LEVEL
