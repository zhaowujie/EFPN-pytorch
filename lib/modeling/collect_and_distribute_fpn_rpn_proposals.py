...
def collect(inputs, is_training):
    ...
    rois = rois[inds, :]# bs=1:1000*5, bs=2:2000*5
    return rois
...
def distribute(rois, label_blobs):
    ...
    #####TODO:!!!
    lvls = fpn_utils.map_rois_to_fpn_levels(rois[:, 1:5], lvl_min - 1, lvl_max)
    ### for coco
    idx1 = np.where(lvls <= lvl_min)[0]
    # ### for TT100K
    # idx1 = np.where(lvls < lvl_min)[0]
    rois1 = rois[idx1, :]
    lvl1 = np.ones((len(idx1))) * (lvl_min - 1)
    lvls[idx1] = lvl_min
    lvls = np.concatenate((lvl1,  lvls))
    rois = np.concatenate((rois1, rois))
    ...
    #####TODO:!!!
    for output_idx, lvl in enumerate(range(lvl_min - 1, lvl_max + 1)):
        idx_lvl = np.where(lvls == lvl)[0]
        blob_roi_level = rois[idx_lvl, :]
        outputs[output_idx + 1] = blob_roi_level
        rois_idx_order = np.concatenate((rois_idx_order, idx_lvl))
    rois_idx_restore = np.argsort(rois_idx_order)
    outputs[-1] = rois_idx_restore.astype(np.int32) # ['rois_idx_restore_int32']
