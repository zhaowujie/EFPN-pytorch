...
def _sample_rois(roidb, im_scale, batch_idx, iou_thresh = None):
    ...
    if iou_thresh is None:
        ...
        # Select foreground RoIs as those with >= FG_THRESH overlap
        fg_inds = np.where(max_overlaps >= iou_thresh)[0]
        # Select background RoIs as those within [BG_THRESH_LO, BG_THRESH_HI)
        bg_inds = np.where((max_overlaps < iou_thresh) &
                           (max_overlaps >= cfg.TRAIN.BG_THRESH_LO))[0]
    ...
    if 'bbox_targets' not in roidb:
        ...
        bbox_targets, bbox_inside_weights = _expand_bbox_targets(bbox_targets, cfg.TRAIN.SR_FINETUNE)
    else:
        bbox_targets, bbox_inside_weights = _expand_bbox_targets(
            roidb['bbox_targets'][keep_inds, :], cfg.TRAIN.SR_FINETUNE)
...
def _expand_bbox_targets(bbox_target_data, sr_finetue=False):
    ...
    """
    # if sr_finetune:
    #     bbox_targets = bbox_target_data[:, 1:5]
    #
    #     bbox_inside_weights = blob_utils.zeros(bbox_targets.shape)
    #     clss = bbox_target_data[:, 0]
    #     inds = np.where(clss > 0)[0]
    #     for ind in inds:
    #         bbox_inside_weights[ind, :] = (1.0, 1.0, 1.0, 1.0)
...
def _add_multilevel_rois(blobs):
    ...
    """
    ###TODO:!!!
    lvl_min = cfg.FPN.ROI_MIN_LEVEL - 1
    ...
    def _distribute_rois_over_fpn_levels(rois_blob_name):
        ...
        # ###TODO:!!!
        # ##for COCO
        # idx1 = np.where(target_lvls <= cfg.FPN.ROI_MIN_LEVEL)[0]
        # target_lvls[idx1] = cfg.FPN.ROI_MIN_LEVEL - 1
        # ####
