...
def get_rpn_blob_names(is_training=True):
    ...
    if is_training:
        ...
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_RPN:
            ...
            start_k = cfg.FPN.RPN_MIN_LEVEL-1 if cfg.FPN.EXTRA_ANCHOR_LAYER else cfg.FPN.RPN_MIN_LEVEL
            for lvl in range(start_k, cfg.FPN.RPN_MAX_LEVEL + 1):
                blob_names += [
...
def add_rpn_blobs(blobs, im_scales, roidb):
    ...
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_RPN:
        ...
        foas = []
        if cfg.FPN.EXTRA_ANCHOR_LAYER:
            field_stride = 2. ** k_min
            anchor_sizes = (cfg.FPN.RPN_ANCHOR_START_SIZE * 2. ** (-1),)
        ...
        for lvl in range(k_min, k_max + 1):
            ...
            # area = (foa.field_of_anchors[:, 2] - foa.field_of_anchors[:, 0]) * (
            #             foa.field_of_anchors[:, 3] - foa.field_of_anchors[:, 1])
            # print('{}:'.format(lvl), area)
            foas.append(foa)
    ...
    for im_i, entry in enumerate(roidb):
        ...
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_RPN:
            ...
            if cfg.FPN.EXTRA_ANCHOR_LAYER:
                ...
                start_k = k_min
            for i, lvl in enumerate(range(start_k, k_max + 1)):
...
def _get_rpn_blobs(im_height, im_width, foas, all_anchors, gt_boxes):
    ...
    if len(gt_boxes) > 0:
        ...
        anchor_by_gt_overlap = box_utils.bbox_overlaps(anchors, gt_boxes) # shape: 141022 * 5


        # Map from anchor to gt box that has highest overlap
        anchor_to_gt_argmax = anchor_by_gt_overlap.argmax(axis=1) # shape: 141022, value:0~4

        # For each anchor, amount of overlap with most overlapping gt box
        anchor_to_gt_max = anchor_by_gt_overlap[np.arange(num_inside),
                                                anchor_to_gt_argmax] #141022


        # Map from gt box to an anchor that has highest overlap
        gt_to_anchor_argmax = anchor_by_gt_overlap.argmax(axis=0) # shape:5, value:0~141022
    ...
    # # print(gt_boxes[anchor_to_gt_argmax[fg_inds], :]) # 7 * 4 ,x1y1x2y2
    # global save_idx
    # gt_num = np.zeros(1000)
    # anchor_gt_num = np.zeros(1000)
    # anchor_num = np.zeros(1000)
    # iou_s = np.zeros(10)
    # iou_m = np.zeros(10)
    # iou_l = np.zeros(10)
    # for box in gt_boxes:
    ...
    #     # if np.sqrt(area) > 999:
    #     #     print(np.sqrt(area))
    #     box_size = min(int(np.sqrt(area)), 999)
    #     assert box_size >= 0
    #     gt_num[box_size] += 1
    # np.save("images/data/anchor_stats/gt_num_{}.npy".format(save_idx), gt_num)
    #
    # for box_i, box in enumerate(gt_boxes[anchor_to_gt_argmax[fg_inds], :]):
    #     area = (box[2] - box[0]) * (box[3] - box[1])
    #     # if np.sqrt(area) > 999:
    #     #     print(np.sqrt(area))
    #     box_size = min(int(np.sqrt(area)), 999)
    #     quan_iou = min(int(10 * anchor_to_gt_max[fg_inds[box_i]]), 9)
    #     if box_size <= 32:
    #         iou_s[quan_iou] += 1
    #     elif box_size <= 96:
    #         iou_m[quan_iou] += 1
    ...
    #
    #     assert box_size >= 0
    #     anchor_gt_num[box_size] += 1
    # np.save('images/data/anchor_stats/iou_s_{}.npy'.format(save_idx), iou_s)
    # np.save('images/data/anchor_stats/iou_m_{}.npy'.format(save_idx), iou_m)
    # np.save('images/data/anchor_stats/iou_l_{}.npy'.format(save_idx), iou_l)
    # np.save("images/data/anchor_stats/anchor_gt_num_{}.npy".format(save_idx), anchor_gt_num)
    #
    # for box in anchors[fg_inds, :]:
    #     area = (box[2] - box[0]) * (box[3] - box[1])
    #     # if np.sqrt(area) > 999:
    #     #     print(np.sqrt(area))
    #     box_size = min(int(np.sqrt(area)), 999)
    #
    #     assert box_size >= 0
    #     anchor_num[box_size] += 1
    # np.save("images/data/anchor_stats/anchor_num_{}.npy".format(save_idx), anchor_num)
    #
    # save_idx += 1
